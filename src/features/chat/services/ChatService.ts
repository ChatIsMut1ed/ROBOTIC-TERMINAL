...
import { modelDetails, OpenAIModel } from "@/global/types/chat/model";
import { ChatConfig } from "@/global/constants/Chat";
import {
  CHAT_COMPLETIONS_ENDPOINT,
  MODELS_ENDPOINT,
} from "@/global/constants/apiEndpoints";
...
export class ChatService {
  ...
  static async mapChatMessagesToCompletionMessages(
    ...
    return messages.map((message) => {
      ...
      if (model.image_support && message.fileDataRef) {
        message.fileDataRef.forEach((fileRef: any) => {
  ...
  static async sendMessage(
    ...
    let headers = {
      ...
      Authorization: `Bearer ${ChatConfig.openapiKey}`,
    };
